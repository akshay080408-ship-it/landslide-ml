...
# Split recipients into admin (technical alert) and citizen (simple alert)
ADMIN_RECIPIENTS = [
    e.strip() for e in os.environ.get(
        'ADMIN_RECIPIENTS', 'disaster.officer@example.com'
    ).split(',')
]
CITIZEN_RECIPIENTS = [
    e.strip() for e in os.environ.get(
        'CITIZEN_RECIPIENTS', 'citizen1@example.com,citizen2@example.com'
    ).split(',')
]

# ── Dynamic alert content generator ──────────────
# WHY: same DANGER alert for all triggers is boring and
# unhelpful. A citizen/officer needs to know WHAT is
# happening (water buildup? ground shaking? anomaly?)
# so they take the right action. Generate alert based
# on the actual sensor readings.
def generate_alert_summary(data):
    """
    Analyze sensor data and generate a brief, actionable
    summary in 3 languages. Returns (trigger_reason, en_msg, hi_msg, as_msg).
    """
    moisture = data.get('moisture', 0)
    accel_mag = data['calculated_features'].get('accel_magnitude', 0)
    moisture_change = data['calculated_features'].get('moisture_change', 0)
    is_anomaly = data.get('is_anomaly', False)

    trigger_reason = []
    if moisture > 3800:
        trigger_reason.append("HIGH soil moisture (water saturation)")
    if accel_mag > 3500:
        trigger_reason.append("STRONG ground vibration/acceleration")
    if moisture_change > 800:
        trigger_reason.append("RAPID moisture increase (rainfall + saturation)")
    if is_anomaly and not trigger_reason:
        trigger_reason.append("UNUSUAL pattern detected")

    trigger_text = " + ".join(trigger_reason) if trigger_reason else "Multiple risk factors"

    en_msg = f"Detected: {trigger_text}"
    hi_msg = {
        "HIGH soil moisture": "मिट्टी में पानी की भारी मात्रा",
        "STRONG ground vibration": "जमीन में तेज कंपन",
        "RAPID moisture increase": "पानी का तेजी से बढ़ना",
        "UNUSUAL pattern": "असामान्य पैटर्न"
    }
    as_msg = {
        "HIGH soil moisture": "মাটিত পানীৰ অধিক পরিমাণ",
        "STRONG ground vibration": "মাটিত তীব্র কম্পন",
        "RAPID moisture increase": "পানীৰ দ্রুত বৃদ্ধি",
        "UNUSUAL pattern": "অস্বাভাৱিক আচরণ"
    }

    hi_summary = " + ".join([hi_msg.get(t, t) for t in trigger_reason]) if trigger_reason else "बहुकारकीय जोखिम"
    as_summary = " + ".join([as_msg.get(t, t) for t in trigger_reason]) if trigger_reason else "বহু-কারক ঝুঁকি"

    return trigger_text, en_msg, hi_summary, as_summary
...
class EmailAlertManager:
    ...
    def send_alert(self, prediction_data):
        ...
        trigger, en_msg, hi_msg, as_msg = generate_alert_summary(prediction_data)

        # Admin alert (technical, for disaster officers)
        admin_body = self._build_admin_body(prediction_data, trigger, en_msg, hi_msg, as_msg)

        # Citizen alert (simple, for community)
        citizen_body = self._build_citizen_body(prediction_data, trigger, en_msg, hi_msg, as_msg)

        if self.test_mode:
            logger.info(f"[TEST MODE] Would email ADMIN: {subject}")
            logger.info(f"[TEST MODE] Would email CITIZENS ({len(CITIZEN_RECIPIENTS)} recipients): {subject}")
            self.emails_sent_count += 1
            return True

        # Send admin alert
        admin_sent = self._actually_send_email_to(subject, admin_body, ADMIN_RECIPIENTS)
        # Send citizen alert
        citizen_sent = self._actually_send_email_to(subject, citizen_body, CITIZEN_RECIPIENTS)

        return admin_sent or citizen_sent

    def _build_subject(self, data):
        return (f"⚠️ LANDSLIDE ALERT - {data['node_id']} - "
                f"{data['confidence']}% Risk")

    def _build_admin_body(self, data, trigger, en_msg, hi_msg, as_msg):
        # Technical alert for disaster management officers
        return f"""
LANDSENSE ML — AUTOMATED DANGER ALERT (OFFICIAL)
लैंडसेंस एमएल — स्वचालित खतरे की चेतावनी (अधिकारी)
লেণ্ডছেন্স এমএল — স্বয়ংক্ৰিয় বিপদ সতৰ্কবাণী (অধিকাৰী)
========================================
TRIGGER / कारण / কাৰণ: {trigger}
{en_msg} | {hi_msg} | {as_msg}

Node / स्थान / স्থান              : {data['node_id']}
...
DETAILED SENSOR READINGS:
  Soil Moisture          : {data.get('moisture', 'N/A')} units
  Acceleration Magnitude : {data['calculated_features']['accel_magnitude']} m/s²
  Moisture Change (5min) : {data['calculated_features']['moisture_change']} units
  Anomaly Score          : {data.get('anomaly_score', 'N/A')}

RECOMMENDATION / सिफारिश / পৰামৰ্শ:
Immediate field verification and emergency response advised.
तुरंत स्थल जांच और आपातकालीन प्रतिक्रिया की आवश्यकता है।
তৎক্ষণাৎ ভূমি পৰিদৰ্শন আৰু জৰুৰী প্ৰতিক্ৰিয়া প্ৰয়োজন।

---
Official Alert | अधिकारी चेतावनी | অধিকাৰী সতৰ্কতা
"""

    def _build_citizen_body(self, data, trigger, en_msg, hi_msg, as_msg):
        # Simple alert for community members
        location_name = NODE_REGISTRY.get(data['node_id'], {}).get('name', data['node_id'])
        return f"""
⚠️ LANDSLIDE ALERT IN YOUR AREA
⚠️ आपके क्षेत्र में लैंडस्लाइड चेतावनी
⚠️ আপোনাৰ এলাকাত ভূমিস্খলন সতৰ্কতা
========================================

📍 LOCATION / स्थान / অঞ্চল: {location_name}
🚨 RISK LEVEL / जोखिम स्तर / ঝুঁকি স्तर: HIGH - TAKE ACTION NOW
⏰ TIME / समय / সময়: {data['timestamp']}

WHAT'S HAPPENING / क्या हो रहा है / কি হৈছে:
{en_msg}
{hi_msg}
{as_msg}

🔴 IMMEDIATE ACTIONS / तुरंत करें / ততক্ষণাৎ কৰক:

1. STAY ALERT
   - Watch for cracks in walls, slopes, or roads
   - Listen for unusual sounds (rumbling, cracking)
   - बड़ी दीवारें, ढलानें में दरारें देखें
   - এন্ধা শক্তিশালী শব্দ (আওয়াজ) শুনুন

2. AVOID RISKY AREAS
   - Don't go near steep slopes
   - Avoid areas under construction
   - खड़ी ढलानों के पास न जाएं
   - নির্মাণ এলাকা এড়ান

3. BE READY TO EVACUATE
   - Keep important documents ready
   - Know your safe route out
   - Tell elderly/children about it
   - महत्वपूर्ण कागजात तैयार रखें
   - নিৰাপদ পথ জানুন

4. CONTACT LOCAL AUTHORITIES
   - Disaster Management: 1234 (replace with your number)
   - Police: 100
   - समर्थन: स्थानीय प्रशासन से संपर्क करें
   - স্থানীয় প্ৰশাসনৰ সৈতে যোগাযোগ কৰক

⏰ This alert was sent automatically by LandSense ML
📱 Follow updates on your local disaster management channel
---
Community Alert | सामुदायिक चेतावनी | সম্প্রদায় সতৰ্কতা
"""
...
"""
    ...
    def _actually_send_email_to(self, subject, body, recipients):
        """Send email to a specific list of recipients (admin or citizen)"""
        if not recipients:
        ...
        try:
            ...
            logger.error(f"Email to {recipients} failed: {e}")
            return False
